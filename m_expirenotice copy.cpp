 ...
 public:
	...
	void OnPreNickExpire(NickAlias *na, bool &expire) override
	{
		...
		if (Anope::CurTime >= notice_at && Anope::CurTime <= notice_at + expiretimeout - 2)
		{
			...
			{
								Anope::string message = Config->GetModule(this).Get<Anope::string>("ns_expiring_memo");
				message = message.replace_all_cs("%n", na->nick);
				message = message.replace_all_cs("%t", Anope::strftime(expire_at, na->nc));

					   MemoServ::service->Send(Config->GetClient("NickServ")->nick, na->nc->display, message, true);
			}
		}
	}

	void OnNickExpire(NickAlias *na) override
	{
		...
		if (ns_notice_memo && (*na->nc->aliases).size() > 1 && !AllAliasesExpiring(na->nc))
		{
						Anope::string message = Config->GetModule(this).Get<Anope::string>("ns_expired_memo");
			message = message.replace_all_cs("%n", na->nick);
	...
	void OnPreChanExpire(ChannelInfo *ci, bool &expire) override
	{
		...
		if (Anope::CurTime >= notice_at && Anope::CurTime <= notice_at + expiretimeout - 2)
		{
			...
			if (cs_notice_memo)
			{
								Anope::string message = Config->GetModule(this).Get<Anope::string>("cs_expiring_memo");
				message = message.replace_all_cs("%c", ci->name);

				if (founder && !AllAliasesExpiring(founder))
				{
					Anope::string founder_message = message;
					founder_message = founder_message.replace_all_cs("%t", Anope::strftime(expire_at, founder));

					   MemoServ::service->Send(Config->GetClient("ChanServ")->nick, founder->display, founder_message, true);
				}
				if (successor && !AllAliasesExpiring(successor))
				{
					Anope::string successor_message = message;
					successor_message = successor_message.replace_all_cs("%t", Anope::strftime(expire_at, successor));

					   MemoServ::service->Send(Config->GetClient("ChanServ")->nick, successor->display, successor_message, true);
				}
			}
		}
	}

	void OnChanExpire(ChannelInfo *ci) override
	{
		/* Do nothing if not enabled or neither notice method is enabled */
		...
		if (cs_notice_memo)
		{
						Anope::string message = Config->GetModule(this).Get<Anope::string>("cs_expired_memo");
			message = message.replace_all_cs("%c", ci->name);

			if (founder && !AllAliasesExpiring(founder))
					   MemoServ::service->Send(Config->GetClient("ChanServ")->nick, founder->display, message, true);
			if (successor && !AllAliasesExpiring(successor))
					   MemoServ::service->Send(Config->GetClient("ChanServ")->nick, successor->display, message, true);
		}
	}
