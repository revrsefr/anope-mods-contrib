...
class JWT final : public SASL::Mechanism {
public:
	JWT(Module *o)
	...
	bool ProcessMessage(SASL::Session *sess, const SASL::Message &m) override
	{
		...
		else if (m.type == "C")
		{
			...
			try
			{
				...
				std::string secret = "9LbwzoQMkbS4yKR2HUMpKZDuZZJ3TsDvp+gza9zYzFpvL+pLk1w07R7ZQ8ZrJGm5Sr9lbFiiPK14JCasK4gQxA==T";
...
class JWTEnhancedPlain final : public SASL::Mechanism
{
private:
	bool looks_like_jwt(const Anope::string &token) const
	{
		// Very basic check - JWT tokens typically have two dots separating header, payload, and signature.
		return token.find('.') != Anope::string::npos && token.rfind('.') != token.find('.');
	}
...
public:
	...
	bool ProcessMessage(SASL::Session *sess, const SASL::Message &m) override
	{
		...
		else if (m.type == "C")
		{
			...
			// We don't support having an authcid that is different from authzid.
			if (!authzid.empty() && authzid != authcid)
				return false;

			Anope::string passwd = message.substr(cpsep + 1);
			if (authcid.empty() || passwd.empty() || !IRCD->IsNickValid(authcid) ||
			    passwd.find_first_of("\r\n\0") != Anope::string::npos)
				return false;
			...
			else
			{
				...
				public:
					SASL::Session *sess;
					SASLIdentifyRequest(Module *m, const Anope::string &acc, const Anope::string &pass, const Anope::string &ip, SASL::Session *s)
						: IdentifyRequest(m, acc, pass, ip), sess(s) {}
				...
				SASLIdentifyRequest *req = new SASLIdentifyRequest(this->owner, authcid, passwd, sess->ip, sess);
...
// External mechanism for certificate-based authentication.
class External final : public SASL::Mechanism
{
private:
	ServiceReference<CertService> certs;

	struct Session final : public SASL::Session
	{
...
public:
	External(Module *o)
		: SASL::Mechanism(o, "EXTERNAL"), certs("CertService", "certs")
	{
		if (!IRCD || !IRCD->CanCertFP)
			throw ModuleException("No CertFP support");
	}
	...
	bool ProcessMessage(SASL::Session *sess, const SASL::Message &m) override
	{
		...
		else if (m.type == "C")
		{
			...
			for (auto it = mysess->certs.begin(); it != mysess->certs.end(); ++it)
			{
				...
				{
					// If using a fallback cert then upgrade it.
...
// Anonymous mechanism.
class Anonymous final : public SASL::Mechanism
{
public:
	Anonymous(Module *o)
...
// Core SASL service that handles sessions and communication.
class SASLService final : public SASL::Service, public Timer
{
private:
...
public:
	SASLService(Module *o)
		: SASL::Service(o), Timer(o, 60, true)
	{
	}
	...
	SASL::Session *GetSession(const Anope::string &uid) override
	{
		...
		return it != sessions.end() ? it->second : NULL;
	}
...
class ModuleSASLJWT final : public Module
{
	SASLService sasl;

	Anonymous anonymous;
	JWTEnhancedPlain plain;  // Using enhanced PLAIN that handles JWT tokens.
	JWT jwt;                 // JWT mechanism.
